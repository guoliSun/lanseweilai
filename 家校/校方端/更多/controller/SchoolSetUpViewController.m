...
@interface SchoolSetUpViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation SchoolSetUpViewController
...
- (void)loadData{
    ...
    NSArray *arrName1 = @[@"个人信息",@"修改密码"];
    NSArray *arrName2 = @[@"清空缓存",@"检查更新",@"关于我们"];
    NSArray *arrName3 = @[@"切换身份",@"退出登录"];
    NSArray *imageName1 = @[@"set_personalinfo",@"set_modifysecret"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section == 0){
        ...
        if (indexPath.row == 0) {
            if(_index == 1){
                SchoolCenterViewController *scVC = [[SchoolCenterViewController alloc]init];
    ...
    if(indexPath.section == 2){
        if (indexPath.row == 0) {
            SchoolStatusSelectViewController *statusSelctVC = [[SchoolStatusSelectViewController alloc]initWithNibName:@"SchoolStatusSelectViewController" bundle:nil];
